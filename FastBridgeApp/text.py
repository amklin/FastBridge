class Text(object):
    ...
    def get_section(self, range_start, range_end):
        ...
        if range_end == "end" or range_end == "start":
            ...
            # for things with one level and 3 level was expected
            print("expected to enter here at line 59")
            try:
                internal_range_end = self.section_linkedlist[next_section(
                    range_end) + ".1.1"]
            except Exception as e:
        ...
        print(type(self.section_linkedlist))
        print(self.sections[internal_range_end])
    ...
    def get_words(self, user_start, user_end):
        ...
        wordlist = [tmp[i] + (self.name,)
                    for i in range(start, end)]  # adds the source text
        return wordlist
...
def next_section(section):
    ...
    working_section = section.split(
        ".")  # so 1.1 = [1, 1], 2 = [2] , 2b.1 = [2b, 1], and 2b = [2b]
    print(working_section)
    length = len(working_section[0])
    # try:
    if(length == 1):
    ...
    #     #     target = target = str(int(working_section[0]) + 1)
    #     target = f"{working_section[0][:-1]}{chr(ord(working_section[0][-1]) + 1)}"

    # try:
    #     target = str(int(working_section[0]) + 1)
    #     print(type(working_section[0]))
    #     print(type(int(working_section[0])))
    #     print(f'target at line 117 in text.py  {target}')
    #     print(type(target))
    # except ValueError: #invalid conversion
    #     # if(len(working_section) == 1)
    #     #     target = target = str(int(working_section[0]) + 1)
